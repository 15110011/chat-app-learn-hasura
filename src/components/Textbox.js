...
export default class Textbox extends React.Component {
  ...
  render() {
    ...
    const client = 'client received from Mutation'
    return (
      <Mutation
        mutation={gql`
          mutation($text:String!, $username: String!) {
        ...
        update={(cache, { data }) => {
          ...
          this.props.mutationCallback({...newMessage});
        }}
      >
        {(insertMessage, { loading, error, data, client }) => {
          const sendMessage = (e) => {
            e.preventDefault();
            insertMessage({
              variables: {
